...
**Updating the key layout**: edit the keymap in the Vial app as usual, export a fresh `.vil`, then run `python3 generate_keymap_from_vil.py path/to/export.vil` from this keymap's directory to regenerate `keymap.json` (it also applies the keycode-alias fixups below automatically). Commit the result and push. It does not touch `rgb_layers.csv`/`ledmap.c` — regenerate those separately if the change needs new colors too.

**Keycode alias quirk**: `.vil` exports use long-form keycode names (`KC_LSHIFT`, `KC_BSPACE`, `KC_SCOLON`, `KC_RSHIFT`, `KC_LBRACKET`, `KC_RBRACKET`, `KC_BSLASH`, `KC_PGDOWN`, `KC_PSCREEN`, …) that this `vial-kb/vial-qmk` fork's keycode alias set doesn't recognize — they fail to compile with "undeclared" errors, even though other long aliases like `KC_ESCAPE`/`KC_ENTER` work fine. `generate_keymap_from_vil.py` has a `KEYCODE_ALIAS_FIXUPS` table for known cases; if a future export hits a new one, add it there (cross-check against `data/constants/keycodes/*.hjson` in a vial-qmk checkout, or just try a build).
...
RGB Matrix brightness defaults to max (`RGB_MATRIX_DEFAULT_VAL` defaults to `RGB_MATRIX_MAXIMUM_BRIGHTNESS`) unless overridden — `config.h` sets it low. This only applies on a fresh/reset EEPROM, not a normal reflash (QMK persists RGB settings across flashes); tune live via Vial's Lighting tab (VialRGB), which works with no dedicated brightness keycodes needed.
...
`CONSOLE_ENABLE` is a stock, cross-platform core QMK feature — deliberately *not* reusing VIA/Vial's own raw HID interface, since `via.c` already owns that channel's `raw_hid_receive()` and pushing unsolicited reports onto it risked corrupting Vial's request/response protocol state while vial.rocks/Vial desktop is open. Confirmed on hardware: layer changes are reported correctly and vial.rocks keeps working normally with the HUD script running at the same time (`hud_console_test.py` — a standalone diagnostic, not the real app, useful on its own for debugging this channel).
